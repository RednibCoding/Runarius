...
/**
 * Handles equipping an inventory item.
 * Client sends: [short slotIndex]
 */
public class CL_InvWearHandler implements IPacketHandler {
    @Override
    public void handle(Socket socket, Buffer data) {
        try {
            ...
            if (player == null) {
                Logger.error("InvWear: player not found for socket");
                return;
            }

            if (slotIndex < 0 || slotIndex >= player.getInventory().size()) {
                Logger.error("InvWear: invalid slot " + slotIndex + " for " + player.getUsername());
                return;
            }
            ...
            if (item.isEquipped()) {
                Logger.debug("InvWear: item already equipped at slot " + slotIndex);
                return;
            }
            ...
            Logger.info(player.getUsername() + " equipped item " + item.getId() + " at slot " + slotIndex);

        } catch (Exception ex) {
            Logger.error("InvWear error: " + ex.getMessage());
        }
    }
}
